...
function initializeAdvancedPortal() {
  // Client dropdown populate karo
  const sel = document.getElementById('advClientSelector');
  if (sel) {
    sel.innerHTML = '<option value="">-- Choose a Client --</option>' +
      STATE.clients.map(c => `<option value="${c.id}">${escapeHtml(c.name)}</option>`).join('');
  }

  // CA selector populate karo (profiles se)
  populateCASelector();

  // Portal cards render karo
  renderAdvPortalCards();

  // UDIN history load karo
  loadUDINHistory();

  // Today's date default set karo
  const docDate = document.getElementById('advDocDate');
  if (docDate && !docDate.value) {
    docDate.value = new Date().toISOString().split('T')[0];
  }
}

async function populateCASelector() {
  const sel = document.getElementById('advCASelector');
  if (!sel) return;
  const profiles = await supabaseQuery('profiles', {
    order: 'full_name.asc',
    filters: 'status=eq.approved'
  });
  sel.innerHTML = '<option value="">-- Select CA --</option>' +
    (profiles || []).map(p => {
      const name = p.full_name || p.email?.split('@')[0] || '';
      return `<option value="${p.id}" data-name="${escapeHtml(name)}">${escapeHtml(name)}</option>`;
    }).join('');
}

function renderAdvPortalCards() {
  const container = document.getElementById('advPortalCardsContainer');
  if (!container) return;

  const portals = [
    { id: 1, name: 'GST Portal', icon: '📊', url: 'https://www.gst.gov.in', color: '#10b981' },
    { id: 2, name: 'MCA Portal', icon: '🏛️', url: 'https://www.mca.gov.in', color: '#3b82f6' },
    { id: 3, name: 'Income Tax', icon: '💰', url: 'https://www.incometax.gov.in', color: '#f59e0b' },
    { id: 4, name: 'TRACES', icon: '🧾', url: 'https://www.tdscpc.gov.in', color: '#8b5cf6' },
    { id: 5, name: 'TDS Portal', icon: '📋', url: 'https://www.tin-nsdl.com', color: '#ef4444' },
    { id: 6, name: 'EPFO', icon: '🏦', url: 'https://www.epfindia.gov.in', color: '#06b6d4' },
    { id: 7, name: 'DGFT', icon: '🌐', url: 'https://www.dgft.gov.in', color: '#ec4899' },
    { id: 8, name: 'ICAI UDIN', icon: '🔢', url: 'https://udin.icai.org', color: '#6366f1' },
  ];

  container.innerHTML = portals.map(p => `
    <div style="background:var(--surface2);border:1.5px solid var(--border);border-radius:12px;padding:18px;cursor:pointer;transition:all .2s"
      onmouseover="this.style.borderColor='${p.color}';this.style.transform='translateY(-2px)'"
      onmouseout="this.style.borderColor='var(--border)';this.style.transform='none'"
      onclick="window.open('${p.url}','_blank')">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
        <div style="width:42px;height:42px;border-radius:10px;background:${p.color}22;display:flex;align-items:center;justify-content:center;font-size:22px">${p.icon}</div>
        <div>
      ...
      <button style="width:100%;padding:8px;background:${p.color};color:#fff;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer"
...
function selectClientAndOpenPortals() {
  ...
  loadUDINHistory();
  showToast('✅ Client selected — portals ready!');
}
...
function displayClientInfo(client) {
  ...
  box.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-top:12px;padding:14px;background:var(--bg);border-radius:10px">
      <div><div style="font-size:11px;color:var(--text-muted)">Client Name</div><div style="font-weight:700;font-size:13px">${escapeHtml(client.name)}</div></div>
      <div><div style="font-size:11px;color:var(--text-muted)">PAN</div><div style="font-weight:600;font-size:13px;font-family:monospace">${escapeHtml(client.pan||'-')}</div></div>
      <div><div style="font-size:11px;color:var(--text-muted)">GSTIN</div><div style="font-weight:600;font-size:13px;font-family:monospace">${escapeHtml(client.gst||'-')}</div></div>
      <div><div style="font-size:11px;color:var(--text-muted)">Type</div><div style="font-weight:600;font-size:13px">${escapeHtml(client.type||'-')}</div></div>
    </div>
  `;
}
...
async function generateAdvancedUDIN() {
  ...
  const caName = document.getElementById('advCASelector')?.selectedOptions[0]?.dataset?.name || '';
  ...
  if (!docDate) { showToast('Please enter document date'); return; }

  // UDIN format: MEMBNO/DDMMYYYY/XXXXXXXX
  const d = new Date(docDate);
  const dd = String(d.getDate()).padStart(2,'0');
  const mm = String(d.getMonth()+1).padStart(2,'0');
  const yyyy = d.getFullYear();
  const random8 = Math.random().toString(36).substring(2,10).toUpperCase();
  const memberNo = caId.substring(0,6).toUpperCase();
  const udin = `${memberNo}${dd}${mm}${yyyy}${random8}`;

  const container = document.getElementById('advUDINResultContainer');
  container.innerHTML = `
    <div style="background:linear-gradient(135deg,#6366f122,#8b5cf622);border:2px solid var(--primary);border-radius:12px;padding:20px;text-align:center">
      <div style="font-size:12px;color:var(--text-muted);margin-bottom:6px">Generated UDIN</div>
      <div style="font-size:22px;font-weight:800;letter-spacing:3px;color:var(--primary);font-family:monospace">${udin}</div>
      <div style="font-size:12px;color:var(--text-muted);margin-top:6px">${caName} • ${docType} • FY ${fy}</div>
      <button onclick="copyToClipboard('${udin}','✅ UDIN copied!')" class="btn-primary" style="margin-top:14px;padding:8px 20px">📋 Copy UDIN</button>
    </div>
  `;
  ...
  const body = {
    ...
    udin_number: udin,
    audit_type: document.getElementById('advAuditType')?.value || '',
    turnover: parseFloat(document.getElementById('advTurnover')?.value) || 0,
    remarks: document.getElementById('advRemarks')?.value.trim() || '',
    filing_status: 'Generated',
    generated_at: new Date().toISOString()
  };

  await supabaseInsert('udin_master', body);
  showToast('✅ UDIN generated & saved!');
  loadUDINHistory();
}
